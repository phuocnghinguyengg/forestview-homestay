...
export default function CalendarDatePicker({ value, onChange, minDate, label = "Chọn ngày" }: { value: string; onChange: (value: string) => void; minDate?: string; label?: string }) {
  ...
  return <div ref={ref} className="relative"><button type="button" onClick={() => setOpen(!open)} className="flex w-full items-center justify-between rounded-none border border-line bg-surface px-3 py-2.5 text-left text-sm transition hover:border-primary"><span className={value ? "text-ink" : "text-neutral-400"}>{value ? format(value) : label}</span><CalendarDays size={17} className="text-primary" /></button>{open && <div className="absolute z-50 mt-2 w-[min(92vw,650px)] rounded-none bg-surface shadow-2xl"><DateRangeCalendar checkIn={value} checkOut="" minDate={minDate} onChange={(a, b) => { onChange(b || a); setOpen(false); }} /></div>}</div>;
}
