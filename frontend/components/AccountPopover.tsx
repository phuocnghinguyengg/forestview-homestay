...
import { ArrowLeft, CalendarDays, LogOut, Settings2, ShieldCheck, UserRound, X } from "lucide-react";
...
export default function AccountPopover({ compact = false }: { compact?: boolean }) {
  ...
  const [view, setView] = useState<"menu" | "account" | "bookings">("menu");
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
            ...
            <div className="flex items-start justify-between bg-ink px-5 py-6 text-white sm:px-6">
              ...
              <div className="flex items-center gap-1">
                {view !== "menu" && <button type="button" onClick={() => setView("menu")} className="rounded-full p-1 text-white/60 hover:bg-white/10 hover:text-white" aria-label="Quay lại"><ArrowLeft size={16} /></button>}
            ...
            {view === "account" && <div className="space-y-4 p-5 sm:p-6">
              <div><p className="text-[11px] font-bold tracking-[0.16em] text-primary uppercase">Thông tin tài khoản</p><h2 className="mt-1 font-display text-2xl text-ink">Hồ sơ của bạn</h2></div>
            ...
            {view === "bookings" && <div className="p-5 sm:p-6">
              <div className="flex items-end justify-between gap-3"><div><p className="text-[11px] font-bold tracking-[0.16em] text-primary uppercase">Lưu trú</p><h2 className="mt-1 font-display text-2xl text-ink">Lịch sử đặt phòng</h2></div><CalendarDays className="text-primary" size={22} /></div>
