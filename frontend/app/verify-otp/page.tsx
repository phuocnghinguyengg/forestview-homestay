...
export default function VerifyOtpPage() {
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setResendMessage("Đã gửi lại mã OTP mới tới email của bạn.");
    } catch (err) {
      setError(getErrorMessage(err, "Không thể gửi lại mã, vui lòng thử lại sau"));
    } finally {
      setResending(false);
    }
  };
  ...
  return (
    <main className="mx-auto flex min-h-[80vh] max-w-md flex-col justify-center px-5 py-12">
      <p className="font-display text-sm italic text-accent">Bước cuối cùng</p>
      <h1 className="mt-1 font-display text-3xl text-ink">Nhập mã xác thực</h1>
      <p className="mt-2 text-sm text-neutral-500">
        Mã gồm 6 chữ số đã được gửi tới <span className="font-medium text-ink">{email}</span>. Mã có hiệu lực trong 10 phút.
      </p>
      ...
      >
        {skipping ? "Đang xử lý..." : "Bỏ qua xác thực (không thể đặt phòng cho đến khi xác thực)"}
      </button>
    </main>
  );
}