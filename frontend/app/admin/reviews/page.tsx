...
export default function AdminReviewsPage() {
  ...
  const filtered = useMemo(
    () => (filter === "ALL" ? reviews : reviews.filter((review) => review.rating === Number(filter))),
    [reviews, filter]
  );

  const remove = async (id: number) => {
    if (!confirm("Ẩn đánh giá này khỏi hệ thống?")) return;
    try {
      await reviewService.removeAdmin(id);
      load();
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          <p className="text-sm font-semibold tracking-wide text-primary uppercase">Voice of guest</p>
        ...
        <div className="rounded-2xl bg-primary/10 px-5 py-3 text-right">
          ...
          <p className="text-xs text-primary">{reviews.length} đánh giá</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div className="min-w-0">
                  <p className="truncate font-medium text-ink">{review.userFullName}</p>
        ...
        {!filtered.length && <p className="py-10 text-center text-sm text-neutral-500">Chưa có đánh giá phù hợp.</p>}
      </div>
    </div>
  );
}
